...
public class GetPopularTags extends AbstractCommand {
	...
	protected void execute() throws Exception {
		List<Tag> tags = getDomain().getTagRepository().getPopularTags(MAX_SIZE);
	...
	private long calculateFontSize(long popularity, long max, long min) {
		...
		if (getLogger().isDebugEnabled()) getLogger().debug("percent: " + percent);

		return Math.round(MIN_FONT_SIZE + ((MAX_FONT_SIZE - MIN_FONT_SIZE) * percent));
	}
}
