...
public class QueryUtils {
	...
	throws Exception {
		registerTaggings(classifiable.getClassification(), classifiable.getId(),
				targetType, jdbcTemplate, tagRepository);
	}

	public static void registerTaggings(
		Classification classification,
		Long targetId, Byte targetType, 
		JdbcTemplate jdbcTemplate,
		TagRepository tagRepository) 
	throws Exception {
	...
	public static List<Long> getParentTagIds(
		...
		return (List<Long>) jdbcTemplate.queryForList(
				...
				new Object[] { targetId, targetType }, Long.class);
	}
	...
	public static List<RawTag> getParentTags(
		...
		return jdbcTemplate.query(sql.toString(), new Object[] { targetId }, tagRowMapper);
	}
	...
	public static Map<Long, RawTag> setOnlyParentTags(
		...
		RawEntityFactory<RawTag> entityFactory) 
	throws Exception {
		return setOnlyParentTags(classifiable.getMutableClassification(),
				classifiable.getId(), targetType, jdbcTemplate, entityFactory);
	}
	...
	public static Map<Long, RawTag> setOnlyParentTags(
			...
			RawEntityFactory<RawTag> entityFactory)
	throws Exception {
		List<RawTag> parentTags = getParentTags(targetId, targetType, jdbcTemplate,
				entityFactory);
	...
	throws Exception {
		...
		if (logger.isDebugEnabled())
			logger.debug("setParentTags for: " + classifiables.values());
		List tagMappings = jdbcTemplate.query(sql.toString(),
				new TagMappingRowMapper(tagRowMapper));
