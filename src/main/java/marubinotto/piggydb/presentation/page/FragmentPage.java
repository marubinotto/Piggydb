...
public class FragmentPage extends FragmentOperations {
	...
	protected PageUrl createThisPageUrl() {
    	...
    	if (this.fragment != null) pageUrl.parameters.put(PN_FRAGMENT_ID, this.fragment.getId());
    	return pageUrl;    	
	}
		...
		if (this.fragment != null || getContext().isAjaxRequest()) {
			...
			if (this.id != null) {
				setRedirectWithMessage(
					HomePage.class, 
		    		getMessage("no-such-fragment", this.id));
			}
			else {
		...
		setRedirectToThisPage(
			getMessage(
				...
				new Object[]{
					...
					this.html.fragmentInMessage(this.fragment)
				},
				false));
		return false;
	}
	...
	protected void setModels() throws Exception {
		...
		getRecentlyViewed().add(new RecentlyViewed.Entity(
			RecentlyViewed.TYPE_FRAGMENT, this.fragment.getId()));
	...
	private void embedCurrentStateInParameters() {
		...
		// For forms
		this.fragmentFormPanel.fragmentForm.add(
			new HiddenField(PN_FRAGMENT_ID, this.fragment.getId()));
		this.subFragmentFormPanel.fragmentForm.add(
			new HiddenField(PN_FRAGMENT_ID, this.fragment.getId()));
		this.superTagForm.add(
			new HiddenField(PN_FRAGMENT_ID, this.fragment.getId()));
