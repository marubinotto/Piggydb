...
import marubinotto.piggydb.presentation.page.control.form.SingleTagForm;
import marubinotto.piggydb.presentation.page.model.RecentlyViewed;
...
public class FilterPage extends FragmentOperations {
	...
	private String setTargetFilter() throws Exception {
		if (this.id != null) {
			...
			if (this.filter == null) return getMessage("FilterPage-no-such-filter", this.id);
		...
		if (tagIds != null || exTagIds != null) {
			if (tagIds != null) {
				for (String tagId : tagIds) {
					Tag tag = getTagRepository().get(Long.parseLong(tagId));
	...
	private Filter createNewFilter() throws Exception {
		Filter filter = getFilterRepository().newInstance(getUser());
	...
	private void initControls() {
		...
		this.filterNameForm.add(new Submit("saveFilter", getMessage("FilterPage-save"), this, "onSaveFilterClick"));
	...
	public boolean onDeleteClick() throws Exception {
		...
		getTransaction().execute(new Procedure() {
			public Object execute(Object input) throws Exception {
				getFilterRepository().delete(getFilter().getId(), getUser());
				return null;
			}
		});
		...
		setRedirectWithMessage(
			HomePage.class, getMessage("FilterPage-completed-delete-filter", this.filter.getName()));
		return false;
	}
	...
	public boolean onSaveFilterClick() throws Exception {
			...
			getTransaction().execute(new Procedure() {
				public Object execute(Object input) throws Exception {
					if (getFilter().getId() != null)
						getFilterRepository().update(getFilter());
					else
						getFilterRepository().register(getFilter());
					return null;
				}
			});
		}
		...
		if (update) {
			...
			setRedirectToThisPage(
				getMessage("FilterPage-filter-saved", this.filter.getName()));
		}
		return false;
	}
	...
	public boolean onAddClassificationTagClick() throws Exception {
		...
		Tag tag = getTagRepository().getByName(tagName);
		if (tag == null) {
	...
	protected void setModels() throws Exception {
		...
		if (this.filter.getId() != null) {
			getRecentlyViewed().add(new RecentlyViewed.Entity(
				RecentlyViewed.TYPE_FILTER, this.filter.getId()));
		}
		
		setCommonSidebarModels();
		
		this.showFilterNameForm = 
			!getUser().isViewer() && (
				isFilterNewInstanceAndModified() || 
				this.renameMode || 
				!this.filterNameForm.isValid()
			);
	}
	...
	private void setRelatedTags() throws Exception {
		...
		RelatedTags relatedTags = getFragmentRepository().getRelatedTags(this.filter);
