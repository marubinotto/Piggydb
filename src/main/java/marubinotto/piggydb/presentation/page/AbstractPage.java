...
import marubinotto.piggydb.presentation.WarSetting;
...
// Workflow 
	...
	public void onInit() {
		super.onInit();
		this.thisPageUrl = createThisPageUrl();	// A target entity object maybe needed
	}
...
// Utilities
	...
	protected String modifyIfGarbledByTomcat(String value) {
		try {
			return WebUtils.modifyIfGarbledByTomcat(
				value, 
	...
	public void setRedirectWithMessage(
...
// Session persistence
    ...
    public static final int PERSISTED_SESSION_MAX_AGE = 60 * 60 * 24 * 7;	// 1 week
    ...
    private void setSessionCookieWhenPersistentCookieIsAboutToBeExpired(HttpSession session) {	
    	if (isPersistentCookieAboutToBeExpired(session)) {
    		...
    		getLogger().debug("The persistent Cookie to be overwritten with a session cookie.");
    	}
    }
